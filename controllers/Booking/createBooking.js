...
const canclebooking = require("../../models/Bookings/cancelBooking");
  ...
  try {
    ...
    if (checkInDateObj < today || checkOutDateObj < today) {
      return res
        ...
        .json({
          message:
            "Check-in or check-out date must not be older than today's date",
        });
    }
    ...
    if (checkInDateObj === checkOutDateObj) {
      return res
        .status(400)
        .json({
          message: "Check-in date and check-out date must not be equal",
        });
    }
    ...
    if (dayDifference < 0) {
      return res
        .status(400)
        .json({ message: "Check out date cannot be before the check-in date" });
    }
    ...
    for (const roomDetail of roomDetails) {
      ...
      for (let i = 0; i <= dayDifference; i++) {
        ...
        if (inventoryEntry && inventoryEntry.manageInventory) {
          const matchingDateEntry = inventoryEntry.manageInventory.find(
            (entry) => entry.date === dateString && entry.isBlocked === "true"
          );
          if (matchingDateEntry) {
            return res
              .status(400)
              .json({
                message: `Inventory is blocked for room type ${roomTypeIdToCheck} on ${dateString}`,
              });
          }
        }
      }
    ...
    for (const roomTypeIdToCheck in roomTypeCounts) {
      ...
      for (const dateToCheck of datesToCheck) {
        ...
        if (
          !inventoryEntry ||
          !inventoryEntry.manageInventory.some(
            (entry) => entry.date === dateToCheck
          )
        ) {
          return res
            .status(400)
            .json({
              message: `Inventory not added for room type ${roomTypeIdToCheck} on ${dateToCheck}`,
            });
        }
        ...
        if (
          roomTypeCounts[roomTypeIdToCheck] > availableInventory ||
          availableInventory === 0
        ) {
          return res
            .status(400)
            .json({
              message: `Not enough inventory available for room type ${roomTypeIdToCheck} on ${dateToCheck}`,
            });
        }
      }
      ...
      return res
        .status(200)
        .json({ message: "Booking saved", bookingId: newBooking.bookingId });
    }
